...
class MultiplePlots:
    '''Display three plots, deprivation, voter turnout, and GE reuslts
    On three different axes'''

    def __init__(self, electoral, deprivation):
        self.electoral = electoral
        self.deprivation = deprivation
        logging.info('OP: Initialised dataframes')

    def plot(self):
        '''Create graphs showing the different data sets
        Figure 1 :First is deprivation by local authority
        Second is GE results, third is GE turnout
        Figure 2: Create a combined, normalised turnout and deprivation overlay to
        show the link, with them displayed separately
        Figure 3: Show the combined turnout and deprivation and GE results'''
        ...
        # Create a figure with three individual plots to show each of the
        # individual data sets separately
        fig_indiv, ax_indiv = plt.subplots(1, 3, figsize=(12, 6))  # Create three subplots

        logging.info('OP: Getting Deprivation data and colourmap')
        custom_cmap, deprivation_data_columnm = self.depriv_plot()
        # Plot the first GeoDataFrame on the first subplot (ax[0])
        # This plot is deprivation by local authority
        self.deprivation.plot(ax=ax_indiv[0], column=deprivation_data_columnm,
                              cmap=custom_cmap, legend=True)

        # Plot the second GeoDataFrame on the second subplot (ax[1])
        # This plot is the 2019 GE results
        logging.info('OP: Plotting scaled electoral results data')
        self.electoral.plot(ax=ax_indiv[1], color=self.electoral['party_colour'],
                            legend=True)

        # Plot the third GeoDataFrame on the second subplot (ax[2])
        # This plot is the 2019 GE voter turnout
        logging.info('OP: Plotting scaled electoral turnout data')
        self.electoral.plot(ax=ax_indiv[2], column=self.electoral['turnout'],
                            legend=True, cmap='viridis_r')

        # Set titles for the subplots
        ax_indiv[0].set_title('2019 Deprivation by Local Authority')
        ax_indiv[1].set_title('2019 GE Results')
        ax_indiv[2].set_title('2019 GE Voter Turnout')

        # Create a new figure for overlaying the different plots to see how
        # they line up with each other
        fig_overlay, ax_overlay = plt.subplots(1, 3, figsize=(12, 6))  # Create three subplots
        ...
        self.electoral.plot(ax=ax_overlay[0], column=self.electoral[elect_normalised],
                            legend=True, cmap='Reds_r', alpha=0.5)
        self.electoral.plot(ax=ax_overlay[1], column=self.electoral['turnout'],
                            legend=True, cmap='Reds_r', alpha=0.5)
        self.deprivation.plot(ax=ax_overlay[0], column=depriv_normalised,
                            legend=True, cmap='Blues', alpha=0.5)
        self.deprivation.plot(ax=ax_overlay[2], column=deprivation_data_columnm,
                            legend=True, cmap='Blues', alpha=0.5)

        # Set titles for the subplots
        ax_overlay[0].set_title('Overlay of Normalised 2019 \n GE Voter Turnout and'
                                + ' Deprivation \n in Local Authorities')
        ax_overlay[1].set_title('Turnout')
        ax_overlay[2].set_title('Deprivation')
        ...
        self.electoral.plot(ax=ax_comparison[0], column=self.electoral[elect_normalised],
                            cmap='Reds_r', alpha=0.5, edgecolor='black', linewidth=0.5)
        self.deprivation.plot(ax=ax_comparison[0], column=depriv_normalised,
                            cmap='Blues', alpha=0.5, edgecolor='black', linewidth=0.5)
        ...
        # Add any additional styling or labels as needed
        plt.tight_layout()  # Ensures proper spacing between subplots
        logging.info('OP: Showing plot...')
        # Display the plot
        #plt.show()

        #return fig_indiv, fig_overlay, fig_comparison
    ...
    def depriv_plot(self):
        ...
        logging.info('OP: Creating deprivation colourmap')
        # Create a new colourmap from above where the lowest value is grey
        cmaplist = [colour_map(i) for i in range(colour_map.N)]
        cmaplist[0] = (0.5, 0.5, 0.5, 1.0)
        custom_cmap = LinearSegmentedColormap.from_list('Custom Colormap', cmaplist, colour_map.N)
        ...
        scalers = ScalarMappable(cmap=custom_cmap, norm=norm)
        scalers.set_array([])

        return custom_cmap, deprivation_data_column
